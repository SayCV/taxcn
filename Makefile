# Makefile for the `taxcn` project
...
ifeq ($(OS),Windows_NT)
BINARY_PATH=$(INSTALL_PREFIX)/taxcn.exe
else
BINARY_PATH=$(INSTALL_PREFIX)/taxcn
endif
...
build: $(INSTALL_PREFIX) generate-optimized
	...
	@go build -ldflags \
	  " -X github.com/saycv/taxcn.BuildCommit=$(BUILD_COMMIT)\
	    -X github.com/saycv/taxcn.BuildTag=$(BUILD_TAG) \
	    -X github.com/saycv/taxcn.BuildTime=$(BUILD_TIME)" \
	  -o $(BINARY_PATH) \
	  *.go
...
ifneq ($(shell git diff --quiet taxcn.go; echo $$?), 0)
